...
The chip cluster is two segments: **ref chips** (local / remote / tag, parsed from `%D` decoration) followed by an **AI-vendor chip** when the commit body's `Co-Authored-By:` trailer matches the 4-vendor whitelist (`anthropic.com` / `openai.com` / `cursor.sh` / `google.com`, exact `@<domain>` suffix). The AI chip uses cyan (color 51) so it reads as a peer of the ref palette but distinct enough to flag AI authorship at a glance. Lookup happens against a 5000-entry LRU keyed by commit hash; a cache miss paints a dim `·` placeholder until the lazy `git show -s --format=%B` lands, at which point the row repaints with either the real chip or nothing (commits without an AI trailer drop the slot entirely so the cyan stays the "AI here" flag). The first 30 commits at the top of the log are prefetched after every stream reload so the initial viewport reads as the AI-attribution surface immediately; everything else fills in via the `Commit` tab's existing detail load as the cursor moves.
