...
Single delete-branch entry: the global `b` modal (centered overlay listing every local branch). Branch creation and rename are handled by worktree workflows (`w` modal) and PR-driven naming; the cockpit doesn't surface modals for them.
