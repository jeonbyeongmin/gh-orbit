...
func TestBuildChipsEmpty(t *testing.T) {
	s, w := buildChips(nil, false, false, aiChipState{fetched: true})
	if s != "" || w != 0 {
		t.Errorf("nil: got (%q, %d), want (\"\", 0)", s, w)
	}
	s, w = buildChips([]string{}, false, false, aiChipState{fetched: true})
	if s != "" || w != 0 {
...
func TestBuildChipsLocalOnly(t *testing.T) {
	s, w := buildChips([]string{"main"}, false, false, aiChipState{fetched: true})
	if s == "" || w == 0 {
...
func TestBuildChipsLocalRemotePairCollapses(t *testing.T) {
	s, _ := buildChips([]string{"HEAD -> main", "origin/main"}, false, false, aiChipState{fetched: true})
	plain := ansi.Strip(s)
...
func TestBuildChipsPairedChipPreservesWidth(t *testing.T) {
	...
	pairedRendered, pairedW := buildChips([]string{"main", "origin/main"}, false, false, aiChipState{fetched: true})
	plainRendered, plainW := buildChips([]string{"feature/x"}, false, false, aiChipState{fetched: true})
...
func TestBuildChipsTagStripsPrefix(t *testing.T) {
	s, _ := buildChips([]string{"tag: v0.0.1"}, false, false, aiChipState{fetched: true})
	plain := ansi.Strip(s)
...
func TestBuildChipsDetachedHeadEmitsNothing(t *testing.T) {
	...
	s, w := buildChips([]string{"HEAD"}, false, false, aiChipState{fetched: true})
	if s != "" || w != 0 {
...
func TestBuildChipsHeadArrowSuppressesHEADToken(t *testing.T) {
	...
	s, _ := buildChips([]string{"HEAD -> main"}, false, false, aiChipState{fetched: true})
	plain := ansi.Strip(s)
	if strings.Contains(plain, "HEAD") {
...
func TestBuildChipsTruncatesAfterTwoWithoutHead(t *testing.T) {
	s, _ := buildChips([]string{"a", "b", "c", "d", "e"}, false, false, aiChipState{fetched: true})
	plain := ansi.Strip(s)
...
func TestBuildChipsTruncatesAfterTwoEvenWithHeadInput(t *testing.T) {
	s, _ := buildChips([]string{"HEAD -> main", "tag: v1", "tag: v2", "tag: v3"}, false, false, aiChipState{fetched: true})
	plain := ansi.Strip(s)
...
func TestBuildChipsDropsSymbolicRemoteHead(t *testing.T) {
	s, _ := buildChips([]string{"HEAD -> develop", "origin/develop", "origin/HEAD"}, false, false, aiChipState{fetched: true})
	plain := ansi.Strip(s)
...
func TestBuildChipsTruncatesLongBranchName(t *testing.T) {
	...
	s, _ := buildChips([]string{long}, false, false, aiChipState{fetched: true})
	plain := ansi.Strip(s)
	if !strings.Contains(plain, "…") {
...
func TestBuildChipsTruncatedNamePreservesPairPrefix(t *testing.T) {
	...
	s, _ := buildChips([]string{"HEAD -> " + long, "origin/" + long}, false, false, aiChipState{fetched: true})
	plain := ansi.Strip(s)
...
func TestBuildChipsSelectedOverridesBackground(t *testing.T) {
	unselected, _ := buildChips([]string{"main"}, false, false, aiChipState{fetched: true})
	selected, _ := buildChips([]string{"main"}, true, false, aiChipState{fetched: true})
...
func TestBuildChipsAIVendorAppendsCyanChip(t *testing.T) {
	rendered, w := buildChips(nil, false, false, aiChipState{fetched: true, vendors: []git.AIVendor{git.VendorAnthropic}})
	if w == 0 {
		t.Fatalf("AI-only row should produce a chip, got width 0")
	}
	plain := ansi.Strip(rendered)
	if !strings.Contains(plain, "anthropic") {
		t.Errorf("expected vendor label, got %q", plain)
	}
	if !strings.Contains(rendered, "48;5;51") {
		t.Errorf("expected cyan (color 51) background ANSI, got %q", rendered)
	}
}

func TestBuildChipsAIPlaceholderEmitsDimDot(t *testing.T) {
	rendered, w := buildChips(nil, false, false, aiChipState{}) // fetched=false
	if w != 1 {
		t.Errorf("placeholder width should be 1 cell, got %d", w)
	}
	plain := ansi.Strip(rendered)
	if plain != "·" {
		t.Errorf("placeholder should be a single dim dot, got %q", plain)
	}
}

func TestBuildChipsAIFetchedNoVendorEmitsNothing(t *testing.T) {
	rendered, w := buildChips(nil, false, false, aiChipState{fetched: true})
	if rendered != "" || w != 0 {
		t.Errorf("fetched + no AI should yield nothing, got (%q, %d)", rendered, w)
	}
}

func TestBuildChipsAIChipFollowsRefChips(t *testing.T) {
	rendered, _ := buildChips([]string{"main"}, false, false, aiChipState{fetched: true, vendors: []git.AIVendor{git.VendorOpenAI}})
	plain := ansi.Strip(rendered)
	mainIdx := strings.Index(plain, "main")
	aiIdx := strings.Index(plain, "openai")
	if mainIdx < 0 || aiIdx < 0 {
		t.Fatalf("expected both labels in %q", plain)
	}
	if mainIdx > aiIdx {
		t.Errorf("ref chip must precede AI chip; got main@%d ai@%d in %q", mainIdx, aiIdx, plain)
	}
}
