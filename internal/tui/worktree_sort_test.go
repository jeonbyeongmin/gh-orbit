...
func TestDashboardWorktreesNaturalOrderWhenSortOff(t *testing.T) {
	m := sortFixture(t)
	// sortByCommit defaults to false.
	got := paths(m.dashboardWorktrees())
...
func TestDashboardWorktreesSortPinsMainAndDescends(t *testing.T) {
	m := sortFixture(t)
...
func TestDashboardWorktreesSortStableTieBreak(t *testing.T) {
...
func TestDashboardWorktreesSortDoesNotMutateOriginal(t *testing.T) {
	m := sortFixture(t)
	m.dashboardFocus.sortByCommit = true
	_ = m.dashboardWorktrees()
	// The backing slice exposed by Worktrees() must stay in git order.
	got := paths(m.refs.Worktrees())
	want := []string{"/main", "/old", "/new", "/unknown"}
	if !eqStrings(got, want) {
		t.Errorf("dashboardWorktrees must not mutate the original slice: got %v, want %v", got, want)
	}
}
